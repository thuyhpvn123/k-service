...
contract KventureProductTest is Test {
    ...
    constructor() {
        ...
        POSNFT.setUsdt(address(USDT_ERC));
        console.log("masterpool:",address(MONEY_POOL));
        vm.stopPrank();
        Register();
    }
    ...
    // function testRegisterLock()public{
    //     vm.warp(currentTime);
    //     // Account 4 == D
    //     vm.startPrank(Deployer);
    //     PO5.RegisterLock(codeRef, codeRef13, 1, address(0x26),bytes32(uint256(4)));
    //     vm.stopPrank();
    //     vm.startPrank(address(0x26));
    //     bytes32 codeRef26 = PO5.GetCodeRef();
    //     vm.stopPrank();
    //     bytes32 idPayment = MONEY_POOL.mIdPayment(address(0x26));
    //     uint256 locktime = MONEY_POOL.mAddressToLockTime(address(0x26));
    //     assertNotEq(bytes32(0),idPayment,"wrong");
    //     assertTrue(locktime>0,"wrong");
    //     assertEq(locktime,timeAfter60Days,"should equal"); //60*24*60*60= seconds in 60days
    //     assertEq(idPayment,bytes32(uint256(4)),"should equal");
    //     console.log("bal la:",POSNFT.balanceOf(address(0x1)));
    // }
    ...
    function testOrder()public{
        ...
        USDT_ERC.burnByOwner(address(Max_out),USDT_ERC.balanceOf(Max_out));
        USDT_ERC.burnByOwner(address(DAO_DT),USDT_ERC.balanceOf(DAO_DT));
        USDT_ERC.burnByOwner(address(0x22),USDT_ERC.balanceOf(address(0x22)));
        ...
        Product memory product = ProductArr[0];
        address NSX = address(0x1991);
        PRODUCT.adminUpdateReward(product.id,27_444*ONE_USDT,200_000*ONE_USDT,NSX);
        vm.stopPrank();
        //case1:
        mintUSDT(address(Deployer), product.memberPrice);
        vm.startPrank(address(Deployer));
        USDT_ERC.approve(address(PRODUCT), product.memberPrice);
        ...
        console.log("Balance DAO before order:",USDT_ERC.balanceOf(DAO_DT));     
        console.log("Balance maxout before order:",USDT_ERC.balanceOf(Max_out));      

        PRODUCT.order(idArr,quaArr,lockArr,codeHashes,delegates,codeRef,address(0x24));
        console.log(USDT_ERC.balanceOf(MTN)+
        USDT_ERC.balanceOf(NSX)+
        USDT_ERC.balanceOf(Max_out)+
        USDT_ERC.balanceOf(address(0x22))+
        USDT_ERC.balanceOf(DAO_DT)+
        PO5.totalRevenues(address(0x22)));
        console.log("bal NSX:",USDT_ERC.balanceOf(NSX));
        console.log("bal U la:",USDT_ERC.balanceOf(address(0x20)));
        console.log("bal Y la:",USDT_ERC.balanceOf(address(0x23)));
        console.log("Balance DAO after order:",USDT_ERC.balanceOf(DAO_DT));    
        console.log("Balance maxout after order:",USDT_ERC.balanceOf(Max_out));      
        // assertEq(USDT_ERC.balanceOf(address(0x23)),1,"wrong");    

        // //case order lock
        // USDT_ERC.approve(address(PRODUCT), product.memberPrice);
        // OrderLockInput memory orderLockInput = OrderLockInput({
        //     id:ProductArr[0].id,
        //     quantity:1,
        //     lock:false,
        //     codeHashes:getCodeHash(1,50000),
        //     delegate:address(0)
        // });
        // bytes32 idPayment = bytes32(uint256(4));
        // OrderLockInput[] memory orderLockInputs = new OrderLockInput[](1);
        // orderLockInputs[0]= orderLockInput;
        // bytes32 idOrder = PRODUCT.orderLock(orderLockInputs,idPayment,address(0x5)); 
        // // string memory idOr = bytes32ToString(idOrder);
        // // console.log("id la:",idOr);   
        // assertNotEq(bytes32(0),idOrder,"wrong");    
        // vm.stopPrank();
